...
class knn:
    ...
    def query(self, pos):
        ...
        for i in range(length):
            j = i
            while j < length:
                Dist[i][j] = Dist[j][i] = self.calc_dist(
...
class KimiaNet_infer:
    def __init__(self, config, dataloader):
        ...
        self.model.features = nn.Sequential(
            self.model.features, nn.AdaptiveAvgPool2d(output_size=(1, 1))
        )
...
class EfficientNet_infer:
    def __init__(self, dataloader):
        ...
        self.model_final = EfficientNet.from_pretrained(
...
class cTransPath_infer:
    def __init__(self, config, dataloader):
        ...
        state_dict = torch.load(self.config["ctrans_model_path"], map_location="cpu")[
            "model"
        ]
...
class GraphConstructor:
    def __init__(
        self, config: OrderedDict, encoder_config: OrderedDict, patch_data, type_info
    ):
        self.config = config
    ...
    def construct_graph(self):
        ...
        het_graph = dgl.to_heterogeneous(
            graph, [str(t) for t in range(self.config["n_node_type"])], ["neg", "pos"]
        )
...
class HyperGraphConstructor:
    def __init__(
        self,
        config: OrderedDict,
        hovernet_config: OrderedDict,
        encoder_config: OrderedDict,
        h5_path=None,
        wsi_path=None,
        pkl_path=None,
        just_extract_embeddings=False,
    ):
        self.config = config
        self.encoder_config = encoder_config
        ...
        if not pkl_path or just_extract_embeddings:
            ...
            if not pkl_path:  # 没有 pkl 文件，则需要用 hovernet 进行预测
                if self.encoder_name == "ctrans":
                    patch_dataset = PatchData(
                        h5_path, wsi, self.hovernet_config["hover_patch_size"]
                    )
                    dataloader = data.DataLoader(
                        patch_dataset,
                        num_workers=4,
                        batch_size=encoder_config["batch_size"]
                        * encoder_config["gpu_num"],
                        shuffle=False,
                    )
                ...
                just_extract_embeddings
            ):  # 仅提取特征，则不需要用 hovernet 进行预测，只使用 encoder 进行特征提取
    ...
    def construct_graph(self, single_edge=False):
        ...
        try:
            ...
            b = np.fromiter(
                chain(
                    *[
                        self.knn_model.query(
                            self.node_pos[v_idx], topn=self.cell_radius
                        )[1:]
                        for v_idx in range(n_patches)
                    ]
                ),
                dtype=int,
            )
            ...
            if single_edge:
                ...
                het_graph = dgl.to_heterogeneous(
                    graph, [str(t) for t in range(self.config["n_node_type"])], ["edge"]
                )
            else:
                # 两种 edgetype
                edge_type = []
        ...
        # print("Het graph created successfully")
        # print("Node types:", het_graph.ntypes)
        # print("Edge types:", het_graph.etypes)

        # het_graph = het_graph.cpu()
        #
        # homo_graph = dgl.graph((edge_spatial[0, :], edge_spatial[1, :]))
        # homo_graph.ndata["feat"] = self.features
        #
        # print("About to return")
        return_tuple = (het_graph, self.HIM, self.info_dict)
        # print("Return tuple created")
        return return_tuple
        return het_graph
    ...
    def process_big_patch(self, MAG, use_all=False):
        ...
        y_sum = 0
        node_pos_dict = {
            (point[0], point[1]): i for i, point in enumerate(self.node_pos)
        }
        _x_dict = {}
        ...
        node_pos_dict = {
            (point[0], point[1]): i for i, point in enumerate(tissue_patch_pos)
        }
        center_point = np.array(
            (x_sum // len(tissue_patch_pos), y_sum // len(tissue_patch_pos))
        )
        ...
        for idx in selected_boundary_index:
            ...
            point_2 = np.array(
                [
                    int((tissue_patch_pos[idx][0] + center_point[0]) * 2 / 3),
                    int((tissue_patch_pos[idx][1] + center_point[1]) * 2 / 3),
                ]
            )
            point_3 = np.array(
                [
                    int((tissue_patch_pos[idx][0] + center_point[0]) / 3),
                    int((tissue_patch_pos[idx][1] + center_point[1]) / 3),
                ]
            )
            distances_to_point2 = np.linalg.norm(
                np.array(tissue_patch_pos) - point_2, axis=1
            )
            distances_to_point3 = np.linalg.norm(
                np.array(tissue_patch_pos) - point_3, axis=1
            )
        ...
        remain_index = list(
            set(all_index)
            - set(selected_boundary_index)
            - set(selected_boundary_index_2)
            - set(selected_boundary_index_3)
            - set(mix_type_index)
        )
        ...
        if self.new_graph_type == "except_topo":
            ...
            selected_boundary_index = random.sample(
                remain_index, len(selected_boundary_index)
            )
            # remain_index =  list(set(remain_index) - set(selected_boundary_index))

            selected_boundary_index_2 = random.sample(
                remain_index, len(selected_boundary_index_2)
            )
            # remain_index =  list(set(remain_index) - set(selected_boundary_index_2))

            selected_boundary_index_3 = random.sample(
                remain_index, len(selected_boundary_index_3)
            )
        ...
        selected_tissue_patch_index = list(
            set(
                selected_boundary_index
                + selected_boundary_index_2
                + selected_boundary_index_3
                + mix_type_index
                + random_select
            )
        )
        selected_features = [
            tissue_features[idx] for idx in selected_tissue_patch_index
        ]
        ...
        hedge_list = [
            i
            for i in range(len(selected_tissue_patch_index))
            for _ in range(self.tissue_radius - 1)
        ]
        _node_list = np.fromiter(
            chain(
                *[
                    self.knn_model.query(
                        selected_features[v_idx], topn=self.tissue_radius
                    )[1:]
                    for v_idx in range(len(selected_features))
                ]
            ),
            dtype=int,
        )
        ...
        H = dglsp.spmatrix(
            torch.LongTensor([node_list, hedge_list]),
            shape=(len(tissue_patch_pos), len(selected_tissue_patch_index) + 4),
        )
        H = H.to_dense()
