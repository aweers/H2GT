...
parser.add_argument(
    "--foldlist", nargs="+", type=int, help="Which fold to use", default=[1, 2, 3, 4, 5]
)
...
def main():
        ...
        if mode == "train":
            if config["train_type"] == "gnn":
                ...
                path = os.path.join(
                    ...
                    config["desc"],
                    str(fold),
                    "model_v{}.pt".format(config["train"]["num_epochs"]),
                )
    ...
    if mode == "train":
        ...
        for idx in range(10, epoch_num + 1, 10):
            ...
            print(
                "epoch:{epoch}, mean:{mean}, std_dev:{std_dev}".format(
                    epoch=idx, mean=mean, std_dev=std_dev
                )
            )
    else:
        print("**" * 20)
