...
parser.add_argument(
    ...
    default="configs/graph_construction_config.yml",
)
...
parser.add_argument(
    "--pkl_path",
    type=str,
    help="The patch type and feature have been processed in advance",
    default=None,
)
...
parser.add_argument(
    ...
    type=bool,
    help="Whether to just extract embeddings",
    default=False,
)
    ...
    if not args.pkl_path:  # 从头开始，包括适用 HoverNet 分割和 Encodr 提取特征
        ...
        for h5_path in h5_paths:
            try:
                ...
                traceback.print_exc()
                print(
                    "Failed to construct {} graph, moves to next WSI image".format(tail)
                )
                pbar.update(1)
            except ZeroDivisionError:
                print(
                    "Failed to construct {} graph,this svs file is to small, moves to next WSI image".format(
                        tail
                    )
                )
                pbar.update(1)
        ...
        for pkl_path in pkl_paths:
            try:
                ...
                graph_constructor = HyperGraphConstructor(
                    ...
                    h5_path=h5_path,
                    wsi_path=wsi_path,
                    just_extract_embeddings=args.just_extract_embeddings,
                )
